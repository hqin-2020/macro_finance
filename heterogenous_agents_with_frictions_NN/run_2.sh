...
# Description: This script evaluate the neural networks for three-dimensional RF / SG / PR economy in Section 5.3.1, 5.3.2, 5.3.3 on the validation set 
...
#SBATCH --job-name=validate_${seed}_${action_name}_${chiUnderline}_${a_e}_${a_h}_${gamma_e}_${gamma_h}_${delta_e}_${delta_h}_${lambda_d}_${nu}_${n_layers}_${points_size}_${units}_${penalization}_${iter_num}
...
python3 ./src/main_variable.py --chiUnderline ${chiUnderline} --a_e ${a_e} --a_h ${a_h} --gamma_e ${gamma_e} --gamma_h ${gamma_h} --rho_e ${rho_e} --rho_h ${rho_h} --nWealth ${nWealth} --nZ ${nZ} --nV ${nV} --V_bar ${V_bar} --sigma_V_norm ${sigma_V_norm} --n_layers ${n_layers} --points_size ${points_size} --iter_num ${iter_num} --seed ${seed} --BFGS_maxiter ${BFGSmaxiter} --BFGS_maxfun ${BFGSmaxfun} --sigma_K_norm ${sigma_K_norm} --sigma_Z_norm ${sigma_Z_norm} --delta_e ${delta_e} --delta_h ${delta_h} --lambda_d ${lambda_d} --nu ${nu} --wMin ${wMin} --wMax ${wMax} --action_name ${action_name} --penalization ${penalization} --units ${units} --shock_expo ${shock_expo}
