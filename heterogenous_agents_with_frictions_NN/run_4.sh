...
# Description: This script computs the shock elasticities for the three-dimensional RF / SG / PR economy in Section 5.3.1, 5.3.2, 5.3.3 using simulation method.

#### Model Configuration Arrays
# Model scenarios and their respective parameter values.
chiUnderlinearray=(1.0 0.2 0.00001)
gamma_earray=(4.0 4.0 4.0)
                                        ...
                                        for iter_num in "${iter_num_array[@]}"; do
                                            for seed in "${seed_array[@]}"; do
                                                for shock_expo in "lower_triangular"; do
                                                    for W_percentile in "${Wpercentile_array[@]}"; do
                                                        ...
                                                        if [ -f ./bash/${action_name}/${shock_expo}/nW_${nWealth}_nZ_${nZ}_nV_${nV}/chiUnderline_${chiUnderline}_a_e_${a_e}_a_h_${a_h}_gamma_e_${gamma_e}_gamma_h_${gamma_h}_delta_e_${delta_e}_delta_h_${delta_h}_lambda_d_${lambda_d}_nu_${nu}/n_layers_${n_layers}_points_size_${points_size}_units_${units}_penalization_${penalization}_iter_num_${iter_num}/simela_${seed}_W_${W_percentile}.sh ]; then
                                                            rm ./bash/${action_name}/${shock_expo}/nW_${nWealth}_nZ_${nZ}_nV_${nV}/chiUnderline_${chiUnderline}_a_e_${a_e}_a_h_${a_h}_gamma_e_${gamma_e}_gamma_h_${gamma_h}_delta_e_${delta_e}_delta_h_${delta_h}_lambda_d_${lambda_d}_nu_${nu}/n_layers_${n_layers}_points_size_${points_size}_units_${units}_penalization_${penalization}_iter_num_${iter_num}/simela_${seed}_W_${W_percentile}.sh
                                                        fi
                                                        ...
                                                        tee -a ./bash/${action_name}/${shock_expo}/nW_${nWealth}_nZ_${nZ}_nV_${nV}/chiUnderline_${chiUnderline}_a_e_${a_e}_a_h_${a_h}_gamma_e_${gamma_e}_gamma_h_${gamma_h}_delta_e_${delta_e}_delta_h_${delta_h}_lambda_d_${lambda_d}_nu_${nu}/n_layers_${n_layers}_points_size_${points_size}_units_${units}_penalization_${penalization}_iter_num_${iter_num}/simela_${seed}_W_${W_percentile}.sh <<EOF
#!/bin/bash

#SBATCH --account=ssd
#SBATCH --job-name=simela_${seed}_W_${W_percentile}_${action_name}_${chiUnderline}_${a_e}_${a_h}_${gamma_e}_${gamma_h}_${delta_e}_${delta_h}_${lambda_d}_${nu}_${n_layers}_${points_size}_${units}_${penalization}_${iter_num}
#SBATCH --output=./job-outs/${action_name}/${shock_expo}/nW_${nWealth}_nZ_${nZ}_nV_${nV}/chiUnderline_${chiUnderline}_a_e_${a_e}_a_h_${a_h}_gamma_e_${gamma_e}_gamma_h_${gamma_h}_delta_e_${delta_e}_delta_h_${delta_h}_lambda_d_${lambda_d}_nu_${nu}/n_layers_${n_layers}_points_size_${points_size}_units_${units}_penalization_${penalization}_iter_num_${iter_num}/simela_${seed}_W_${W_percentile}.out
#SBATCH --error=./job-outs/${action_name}/${shock_expo}/nW_${nWealth}_nZ_${nZ}_nV_${nV}/chiUnderline_${chiUnderline}_a_e_${a_e}_a_h_${a_h}_gamma_e_${gamma_e}_gamma_h_${gamma_h}_delta_e_${delta_e}_delta_h_${delta_h}_lambda_d_${lambda_d}_nu_${nu}/n_layers_${n_layers}_points_size_${points_size}_units_${units}_penalization_${penalization}_iter_num_${iter_num}/simela_${seed}_W_${W_percentile}.err
...
python3 ./src/main_sim_shock_elasticity.py --chiUnderline ${chiUnderline} --a_e ${a_e} --a_h ${a_h} --gamma_e ${gamma_e} --gamma_h ${gamma_h} --rho_e ${rho_e} --rho_h ${rho_h} --nWealth ${nWealth} --nZ ${nZ} --nV ${nV} --V_bar ${V_bar} --sigma_V_norm ${sigma_V_norm} --n_layers ${n_layers} --points_size ${points_size} --iter_num ${iter_num} --seed ${seed} --BFGS_maxiter ${BFGSmaxiter} --BFGS_maxfun ${BFGSmaxfun} --sigma_K_norm ${sigma_K_norm} --sigma_Z_norm ${sigma_Z_norm} --delta_e ${delta_e} --delta_h ${delta_h} --lambda_d ${lambda_d} --nu ${nu} --wMin ${wMin} --wMax ${wMax} --action_name ${action_name} --penalization ${penalization} --units ${units} --shock_expo ${shock_expo} --W_percentile ${W_percentile}
